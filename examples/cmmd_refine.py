...
def main(out_path: Path | None = None) -> tuple[float, float]:
    ...
    x = generator.multivariate_normal(
        np.zeros(num_features), feature_sd * np.eye(num_features), num_data_points
    )
    ...
    data = ArrayData.load(original_data=data)
    ...
    refiner = RefineCMMD(
        random_key=build_key, unique=True, batch_size=None, order="random"
    )
    ...
    greedy_cmmd.fit(original_data=data, strategy=SizeReduce(coreset_size=coreset_size))
    ...
    # Compute the CMMD between the original data and the coreset created via GreedyCMMD
    metric_object = CMMD(
