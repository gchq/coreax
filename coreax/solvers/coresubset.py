...
def _initial_coresubset(coreset_size: int, dataset: _Data) -> Coresubset[_Data]:
    """Generate an coresubset of the specified size with all indices set to zero."""
...
def _greedy_kernel_selection(
    ...
    :return: Coresubset where each point in
    """
    data = jnp.asarray(coresubset.pre_coreset_data)
    ...
    padded_coresubset = jtu.tree_map(
        lambda x: jnp.pad(x, (0, max(0, output_size - len(coresubset)))),
        coresubset,
    )
...
class KernelHerding(RefinementSolver[_Data, HerdingState], ExplicitSizeSolver):
    ...
    def refine(
        ...
        def selection_function(i: int, _kernel_similarity_penalty: ArrayLike) -> Array:
            return jnp.argmax(gramian_row_mean - _kernel_similarity_penalty / (i + 1))
...
class RPCholesky(CoresubsetSolver[_Data, RPCholeskyState], ExplicitSizeSolver):
    ...
    def reduce(
        ...
        output_state = jax.lax.fori_loop(
            1, self.coreset_size + 2, _greedy_body, init_state
        )
...
class SteinThinning(RefinementSolver[_Data, None], ExplicitSizeSolver):
    ...
    def refine(
        ...
        :return: a refined coresubset and relevant intermediate solver state information
        """
        x = coresubset.pre_coreset_data.data
        ...
        def selection_function(i: int, _kernel_similarity_penalty: ArrayLike) -> Array:
            ...
            if self.regularise:
                stein_kernel_diagonal += _laplace_positive(x)
                kde = jsp.stats.gaussian_kde(x.T, bw_method=bandwidth_method)
            ...
            return jnp.argmin(kernel_stein_discrepancy)
