...
def _initial_coresubset(coreset_size: int, dataset: _Data) -> Coresubset[_Data]:
    """Generate a coresubset of the specified size with all indices set to zero."""
...
def _greedy_kernel_selection(
    ...
    """
    data = jnp.atleast_2d(jnp.asarray(coresubset.pre_coreset_data))
...
class SteinThinning(RefinementSolver[_Data, None], ExplicitSizeSolver):
    ...
    def refine(
        ...
        def _laplace_positive(x_):
            r"""Evaluate the Laplace positive operator  :math:`\Delta^+ \log p(x)`."""
