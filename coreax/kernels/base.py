...
class ScalarValuedKernel(eqx.Module):  # noqa: PLR0904
    ...
    def grad_x_elementwise(
        self,
        x: Shaped[ArrayLike, " d"],
        y: Shaped[ArrayLike, " d"],
    ) -> Shaped[Array, " d"]:
        r"""
    ...
    def compute_mean(
        ...
        *,
        block_size: int | None | tuple[int | None, int | None] = None,
        unroll: int | bool | tuple[int | bool, int | bool] = 1,
    ) -> Shaped[Array, " n"] | Shaped[Array, " m"] | Scalar:
        r"""
        ...
        def block_sum(accumulated_sum: Scalar, x_block: Data) -> tuple[Array, Array]:
            """Block reduce/accumulate over ``x``."""

            def slice_sum(
                accumulated_sum: Scalar, y_block: Data
            ) -> tuple[Array, Array]:
