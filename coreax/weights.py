...
elements of it. These weights are determined by optimising the metric of interest, that
...
All methods implement :class:`WeightsOptimiser` and must have a :meth:`solve` method
that, given two datasets, returns an array of weights such that a metric of interest is
optimised when these weights are applied to the dataset.
"""
