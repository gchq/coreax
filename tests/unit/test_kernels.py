...
class TestAnisotropicKernel(
    BaseKernelTest[AnisotropicKernel],
    KernelMeanTest[AnisotropicKernel],
    KernelGradientTest[AnisotropicKernel],
):
    """Test ``coreax.kernels.AnisotropicKernel``."""

    @pytest.fixture(scope="class")
    def kernel(self) -> AnisotropicKernel:
        """Return a scalar-scaled linear kernel for shared kernel tests."""
        return AnisotropicKernel(
            LinearKernel(output_scale=1.0, constant=0.0),
            2.0,
        )

    @pytest.fixture
    def problem(
        self, request: pytest.FixtureRequest, kernel: AnisotropicKernel
    ) -> _Problem:
        """Return a two-feature problem for the shared compute test."""
        del request
        x = np.array([[0.0, 1.0], [2.0, -1.0]])
        y = np.array([[1.0, 3.0], [-1.0, 2.0], [0.5, 0.5]])
        scale = float(kernel.length_scale)
        expected = (x / scale) @ (y / scale).T
        return _Problem(x, y, expected, kernel)

    @override
    def expected_grad_x(
        self, x: Array, y: Array, kernel: AnisotropicKernel
    ) -> np.ndarray:
        """Compute the expected gradient with respect to ``x``."""
        scale_sq = float(kernel.length_scale) ** 2
        x_array, y_array = np.asarray(x), np.asarray(y)
        if x_array.ndim == 1:
            return y_array / scale_sq
        return np.broadcast_to(
            y_array[None, :, :] / scale_sq,
            (x_array.shape[0], y_array.shape[0], y_array.shape[1]),
        )
    ...
    def expected_grad_y(
        ...
        """Compute the expected gradient with respect to ``y``."""
        scale_sq = float(kernel.length_scale) ** 2
        x_array, y_array = np.asarray(x), np.asarray(y)
        if x_array.ndim == 1:
            return x_array / scale_sq
        return np.broadcast_to(
            x_array[:, None, :] / scale_sq,
            (x_array.shape[0], y_array.shape[0], x_array.shape[1]),
        )
    ...
    def expected_divergence_x_grad_y(
        ...
        """Compute the expected mixed-gradient divergence."""
        x_array, y_array = np.asarray(x), np.asarray(y)
        dimension = x_array.shape[-1] if x_array.ndim > 1 else x_array.size
        divergence = dimension / float(kernel.length_scale) ** 2
        if x_array.ndim == 1:
            return np.asarray(divergence)
        return np.full((x_array.shape[0], y_array.shape[0]), divergence)

    @pytest.mark.parametrize("scale", [2.0, [0.5, 2.0]])
    def test_gaussian_values(
        self, jit_variant: Callable[[Callable], Callable], scale: ArrayLike
    ) -> None:
    ...
    def test_existing_kernels(
        self, jit_variant: Callable[[Callable], Callable], base: ScalarValuedKernel
    ) -> None:
    ...
    def test_gaussian_derivatives(
        self, jit_variant: Callable[[Callable], Callable], coincident: bool
    ) -> None:
    ...
    def test_tree_update(self, jit_variant: Callable[[Callable], Callable]) -> None:
        """Retain the scale as a replaceable PyTree leaf."""
        kernel = AnisotropicKernel(SquaredExponentialKernel(), jnp.ones(2))
        updated = eqx.tree_at(
            lambda item: item.length_scale, kernel, jnp.array([0.5, 2.0])
        )
        first, second = jnp.zeros(2), jnp.ones(2)
        expected = jnp.exp(
            -jnp.sum((first - second) ** 2 / updated.length_scale**2) / 2
        )
        np.testing.assert_allclose(
    ...
    def test_weighted_metric(self, jit_variant: Callable[[Callable], Callable]) -> None:
        """Preserve weighted metrics under an equivalent input transformation."""
    ...
    def test_scalar_inputs(
        ...
        np.testing.assert_allclose(
            jit_variant(kernel.divergence_x_grad_y_elementwise)(1.0, 2.0),
            expected_trace,
        )
        np.testing.assert_allclose(
    ...
    def test_invalid_scale_shapes(self, scale: ArrayLike) -> None:
