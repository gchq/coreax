...
# We require more variables and attributes for the supervised case
# pylint: disable=too-many-instance-attributes
# pylint: disable=too-many-locals
class TestGreedyCMMD(unittest.TestCase):
    """
    Tests related to the RPCholesky class defined in coresubset.py.
    """
    ...
    def setUp(self):
        ...
        # Size of the coreset to generate
        self.coreset_size = 2
        # Set random keys
        self.random_data_generation_key = 0
        self.random_key = random.key(0)
        ...
        # Define some generic data for use in input validation
        generator = np.random.default_rng(self.random_data_generation_key)
        feature_array = generator.random((self.num_data_points, self.feature_dimension))
        response_array = generator.random(
            (self.num_data_points, self.response_dimension)
        )
        original_data = jnp.hstack((feature_array, response_array))
        self.generic_data = coreax.data.ArrayData.load(original_data)
    ...
    def test_tree_flatten(self) -> None:
        ...
        coresubset_object_not_random = coreax.coresubset.GreedyCMMD(
            self.random_key,
        ...
        # Set attributes on the object to ensure actual values are returned
        coresubset_object_not_random.kernel_matrix_row_sum_mean = None
        coresubset_object_not_random.feature_gramian = jnp.zeros((2, 2))
        coresubset_object_not_random.response_gramian = jnp.zeros((2, 2))
        coresubset_object_not_random.training_cme = jnp.zeros((2, 2))
        coresubset_object_not_random.kernel = None
        coresubset_object_not_random.regularisation_parameter = 1e-4
        coresubset_object_not_random.unique = False
        coresubset_object_not_random.batch_size = 1
        coresubset_object_not_random.coreset_indices = jnp.zeros(1, dtype=jnp.int32)
        coresubset_object_not_random.coreset = jnp.zeros([2, 3])
        coresubset_object_not_random.refine_method = "ABC"
        coresubset_object_not_random.weights_optimiser = "DEF"
        ...
        self.assertEqual(len(output_children), 8)
        self.assertEqual(output_children[0], self.random_key)
        self.assertEqual(output_children[1], feature_kernel)
        self.assertEqual(output_children[2], response_kernel)
        np.testing.assert_array_equal(output_children[3], jnp.zeros(1, dtype=jnp.int32))
        np.testing.assert_array_equal(output_children[4], jnp.zeros([2, 3]))
        np.testing.assert_array_equal(output_children[5], jnp.zeros((2, 2)))
        np.testing.assert_array_equal(output_children[6], jnp.zeros((2, 2)))
        np.testing.assert_array_equal(output_children[7], jnp.zeros((2, 2)))

        self.assertDictEqual(
            output_aux_data,
            {
                "num_feature_dimensions": 1,
                "regularisation_parameter": 1e-4,
                "unique": False,
                "batch_size": 1,
                "refine_method": "ABC",
                "unused_kernel": None,
    ...
    def test_fit_comparison_to_random_and_refined(self) -> None:
        """
        Test the fit method of the GreedyCMMD class with a simple example.

        The test checks that a coreset generated via GreedyCMMD has an improved quality
        (measured by conditional maximum mean discrepancy) than one generated by random
        sampling. We further check that if the coreset generated by GreedyCMMD is
        refined, the quality improves yet again.
        """
        # Define specific test instance setup
        feature_kernel = coreax.kernel.SquaredExponentialKernel()
        ...
        # Define some data - sufficiently large that we would not expect a random sample
        # to typically compete with a GreedyCMMD approach
        generator = np.random.default_rng(self.random_data_generation_key)
        original_data = jnp.hstack(
            (
                generator.random((num_data_points, feature_dimension)),
                generator.random((num_data_points, response_dimension)),
            )
        )
        data = coreax.data.ArrayData.load(original_data)

        # Create a GreedyCMMD object
        refine_method = coreax.refine.RefineCMMD(self.random_key, unique=True)
        coresubset_object_not_random = coreax.coresubset.GreedyCMMD(
            self.random_key,
            feature_kernel=feature_kernel,
            response_kernel=response_kernel,
            num_feature_dimensions=feature_dimension,
            refine_method=refine_method,
            unique=True,
        )

        # Apply GreedyCMMD on the dataset, and record the coreset for comparison
        coresubset_object_not_random.fit(
            original_data=data, strategy=coreax.reduction.SizeReduce(coreset_size)
        )
        fitted_coresubset = coresubset_object_not_random.coreset
        ...
        not_random_metric = metric.compute(data.original_data, fitted_coresubset)
        random_metric = metric.compute(data.original_data, random_coreset)
        self.assertLess(float(not_random_metric), float(random_metric))

        # Create a coreset via GreedyCMMD with refinement, to check refinement
        # improves the coreset quality
        coresubset_object_not_random.refine()
        refined_fitted_coresubset = coresubset_object_not_random.coreset

        # Compare quality of refined coreset to non-refined coreset
        refined_not_random_metric = metric.compute(
            data.original_data, refined_fitted_coresubset
        )
        self.assertLess(float(refined_not_random_metric), float(not_random_metric))

    def test_fit_comparison_to_random_and_refined_not_unique(self) -> None:
        """
        Test the non-unique fit method of the GreedyCMMD class with a simple example.

        The test checks that a coreset generated via GreedyCMMD has an improved quality
        (measured by conditional maximum mean discrepancy) than one generated by random
        sampling. We further check that if the coreset generated by GreedyCMMD is
        non-uniquely refined, the quality improves yet again.
        """
        # Define specific test instance setup
        feature_kernel = coreax.kernel.SquaredExponentialKernel()
        response_kernel = coreax.kernel.SquaredExponentialKernel()
        ...
        # Create a GreedyCMMD object
        refine_method = coreax.refine.RefineCMMD(self.random_key, unique=False)
        coresubset_object_not_random = coreax.coresubset.GreedyCMMD(
            self.random_key,
            feature_kernel=feature_kernel,
            response_kernel=response_kernel,
    ...
    def test_greedy_cmmd_invalid_refine_method(self):
        """
        Test the class GreedyCMMD when given an invalid refine_method object.
        """
        # Define a GreedyCMMD object with the invalid refine_method - note that
        # InvalidKernel also does not have a refine method, so suits the purpose of
        # this test
    ...
    def test_greedy_cmmd_fit_zero_coreset_size(self):
        """
        Test how GreedyCMMD performs when given a zero value of coreset_size.
        """
        # Define a GreedyCMMD object
        greedy_cmmd_object = coreax.coresubset.GreedyCMMD(
            random_key=self.random_key,
            feature_kernel=coreax.kernel.SquaredExponentialKernel(),
            response_kernel=coreax.kernel.SquaredExponentialKernel(),
        ...
        # Call the fit method with a coreset size of 0
        with self.assertRaises(ValueError) as error_raised:
            greedy_cmmd_object.fit(
                original_data=self.generic_data,
                strategy=coreax.reduction.SizeReduce(coreset_size=0),
    ...
    def test_greedy_cmmd_fit_negative_coreset_size(self):
        """
        Test how GreedyCMMD performs when given a negative value of coreset_size.
        """
        # Define a GreedyCMMD object
        greedy_cmmd_object = coreax.coresubset.GreedyCMMD(
            random_key=self.random_key,
        ...
        # Call the fit method with a coreset size of -1
        with self.assertRaises(ValueError) as error_raised:
            greedy_cmmd_object.fit(
                original_data=self.generic_data,
                strategy=coreax.reduction.SizeReduce(coreset_size=-1),
            )
        self.assertEqual(
            error_raised.exception.args[0],
            "coreset_size must be a positive integer",
    ...
    def test_greedy_cmmd_fit_float_coreset_size(self):
        """
        Test how GreedyCMMD performs when given a float value of coreset_size.
        """
        # Define a GreedyCMMD object
        greedy_cmmd_object = coreax.coresubset.GreedyCMMD(
            random_key=self.random_key,
        ...
        # Call the fit method with a coreset size of 1.0
        with self.assertRaises(ValueError) as error_raised:
            greedy_cmmd_object.fit(
                original_data=self.generic_data,
                strategy=coreax.reduction.SizeReduce(coreset_size=1.0),
            )
        self.assertEqual(
            error_raised.exception.args[0],
            "coreset_size must be a positive integer",
    ...
    def test_greedy_cmmd_fit_invalid_size_reduce(self):
        """
        Test how GreedyCMMD performs when given an invalid reduction strategy.
        """
        # Define a GreedyCMMD object
        greedy_cmmd_object = coreax.coresubset.GreedyCMMD(
            random_key=self.random_key,
    ...
    def test_greedy_cmmd_fit_invalid_data(self):
        """
        Test how GreedyCMMD performs when given an invalid data.
        """
        # Define a GreedyCMMD object
        greedy_cmmd_object = coreax.coresubset.GreedyCMMD(
            random_key=self.random_key,
