...
class TestGradientFlow(ExplicitSizeSolverTest):
    """Test cases for :class:`coreax.solvers.GradientFlow`."""

    OVERSIZE_MSG = "'coreset_size' cannot exceed the dataset size"
    ...
    def initial_coreset(self) -> PseudoCoreset[Data]:
        ...
        target = Data(
            jnp.array([[0.0, 1.0], [4.0, 3.0], [10.0, -2.0]]),
            jnp.array([1.0, 3.0, 0.0]),
        )
    ...
    def test_linear_step(
        ...
        np.testing.assert_array_equal(
            result.points.weights, initial_coreset.points.weights
        )
        np.testing.assert_array_equal(
    ...
    def test_gaussian_step(
        ...
        result, state = jit_variant(solver.refine)(
            initial_coreset, GradientFlowState(key)
        )
        next_key, noise_key = jr.split(key)
        points = np.asarray(initial_coreset.points.data)
        evaluation = points + noise_scale * np.asarray(
            jr.normal(noise_key, points.shape)
        )
        repulsion = np.einsum(
        ...
        np.testing.assert_array_equal(
            jr.key_data(state.random_key), jr.key_data(next_key)
        )
    ...
    def test_zero_update_preserves_points(
        ...
        """Check zero learning rate or iteration count preserves particle values."""
        solver = GradientFlow(
            2,
            jr.key(0),
            SquaredExponentialKernel(),
    ...
    def test_sampling_uses_weights_and_is_reproducible(
        ...
        """Check sampling excludes zero-mass points and can repeat active points."""
    ...
    def test_weight_rescaling_and_zero_mass_do_not_change_flow(
        ...
        solver = GradientFlow(
            2, jr.key(5), SquaredExponentialKernel(), num_iterations=4
        )
    ...
    def test_small_steps_reduce_mmd(
    ...
    def test_rejects_invalid_weights(
        self, jit_variant: Callable[[Callable], Callable], weights: list[float]
    ) -> None:
    ...
    def test_rejects_non_finite_coordinates(self, value: float) -> None:
    ...
    def test_invalid_parameters(self, name: str, value: float) -> None:
    ...
    def test_rejects_invalid_optimiser(
        self,
    ) -> None:
    ...
    def test_rejects_invalid_noise_schedule_values(
        self, jit_variant: Callable[[Callable], Callable], scheduled_scale: float
    ) -> None:
        """Validate noise schedules at the optimisation step where they are used."""
        solver = GradientFlow(
            1,
            jr.key(0),
            LinearKernel(),
    ...
    def test_invalid_shapes_and_supervision(
        self,
    ) -> None:
    ...
    def test_mixed_coordinate_and_weight_precision(
        ...
        try:
            ...
            np.testing.assert_array_equal(
                result.points.weights, particles.points.weights
            )
        finally:
