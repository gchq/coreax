...
class TestJMMD:
    ...
    def test_jmmd_random_data(
        ...
        kernel_nn = feature_kernel.compute(d1.data, d1.data) * response_kernel.compute(
            d1.supervision, d1.supervision
        )
        kernel_mm = feature_kernel.compute(d2.data, d2.data) * response_kernel.compute(
            d2.supervision, d2.supervision
        )
        kernel_nm = feature_kernel.compute(d1.data, d2.data) * response_kernel.compute(
            d1.supervision, d2.supervision
        )
        if mode == "weighted":
            assert isinstance(d1.weights, Array)
            assert isinstance(d2.weights, Array)
            weights_nn = d1.weights[..., None] * d1.weights[None, ...]
            weights_mm = d2.weights[..., None] * d2.weights[None, ...]
            weights_nm = d1.weights[..., None] * d2.weights[None, ...]
            expected_mmd = jnp.sqrt(
                jnp.average(kernel_nn, weights=weights_nn)
                + jnp.average(kernel_mm, weights=weights_mm)
                - 2 * jnp.average(kernel_nm, weights=weights_nm)
            ...
            d1, d2 = (
                SupervisedData(d1.data, d1.supervision),
                SupervisedData(d2.data, d2.supervision),
            )
            expected_mmd = jnp.sqrt(
                jnp.mean(kernel_nn) + jnp.mean(kernel_mm) - 2 * jnp.mean(kernel_nm)
            )
