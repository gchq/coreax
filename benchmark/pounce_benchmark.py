...
"""
Benchmark different coreset algorithms on frame data extracted from a GIF.

The benchmarking process follows these steps:
1. Load an input GIF and preprocess its frames.
2. Reshape the frame data and apply PCA for dimensionality reduction.
...
def benchmark_coreset_algorithms(
    ...
    solvers = initialise_solvers(pca_data, random.PRNGKey(45))

    for solver_func in solvers:
        solver_name, solver = solver_func(coreset_size)
