...
### Changed
 - `ScoreNetwork` now requires `num_random_vectors` and `num_noise_models` to be provided as positive integers, and allows the `optimiser` to be a string whose name is an `optax` optimiser.
